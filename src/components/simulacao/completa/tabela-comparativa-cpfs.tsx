    ...
    <div className="space-y-4">
        ...
        <Table>
          ...
          <TableBody>
            {todasLinhas.map((linha, idx) => {
              ...
              const outrasLinhasMesmoBanco = todasLinhas.filter(
                (l) => l.nome_banco === linha.nome_banco && l.id !== linha.id,
              );
              const empatouTaxa = outrasLinhasMesmoBanco.some(
                (l) =>
                  l.taxa_juros_ano === linha.taxa_juros_ano &&
                  l.status_banco === "simulada" &&
                  linha.status_banco === "simulada",
              );
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        ...
                        style={{ color: corDoBanco(linha.nome_banco) }}
                      >
                        {linha.nome_banco}
                      </span>
                    </div>
      ...
      {!simuladas.length && (
        <div className="flex items-center gap-2 rounded-lg bg-orange-500/10 p-3 text-xs text-orange-600 border border-orange-500/20">
          ...
          Nenhuma combinação aprovada até o momento. Verifique os motivos de recusa nas situações
          individuais.
        </div>
      )}
    </div>
