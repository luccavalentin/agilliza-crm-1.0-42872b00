...
import { PRIORIDADE, STATUS_TAREFA, type Prioridade } from "@/components/operacional/status";
...
/** Cor sólida de cada status, no espírito das "status columns" do Monday. */
...
const COLS =
  "grid-cols-[28px_minmax(0,1fr)] md:grid-cols-[28px_minmax(0,1fr)_136px_148px_120px_150px_40px]";
...
/** Pílula de status sólida com troca rápida — padrão board profissional. */
function StatusPill({
  status,
  onChange,
}: {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          className="group/st flex h-8 w-full items-center justify-center gap-1 rounded-md px-2 text-[12px] font-semibold text-white shadow-[inset_0_-2px_0_rgba(0,0,0,0.12)] transition-[filter,transform] hover:brightness-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
...
/** Board de tarefas em grupos colapsáveis com colunas — inspirado em ferramentas tipo Monday. */
    ...
    <div className="space-y-5">
      {grupos.map((g) => {
        ...
        return (
          ...
          >
            {/* Cabeçalho do grupo */}
            <header className="flex items-center gap-2 border-b border-border/70 bg-[color-mix(in_oklab,var(--op-accent)_7%,var(--card))] px-3 py-2.5">
              <button
                type="button"
                onClick={() => setFechados((f) => ({ ...f, [g.id]: aberto }))}
                aria-label={aberto ? "Recolher grupo" : "Expandir grupo"}
                className="grid size-6 place-items-center rounded-md text-[color:var(--op-accent)] transition-colors hover:bg-accent"
              >
                <ChevronDown
            ...
            {aberto && (
              <>
                ...
                <div
                  className={cn(
                    "hidden gap-3 border-b border-border/60 bg-muted/40 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground md:grid",
                    COLS,
                  )}
                >
                  <span />
                ...
                <ul className="divide-y divide-border/60">
                  {g.tarefas.map((t) => {
                    ...
                    const prio = PRIORIDADE[t.prioridade as Prioridade] ?? PRIORIDADE.p3;
                    return (
                      <li
                        key={t.id}
                        className={cn(
                          "group relative grid items-center gap-3 px-3 py-2.5 transition-colors hover:bg-accent/40",
                          COLS,
                        )}
                      >
                        <span
                        ...
                        <div className="hidden justify-center md:flex">
                          <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-background px-2.5 py-1 text-[11px] font-semibold text-muted-foreground">
                            <span className={cn("h-2.5 w-[3px] rounded-full", prio.bar)} />
                            {prio.label}
                          </span>
                        </div>
                        ...
                        <div
                          className={cn(
                            "hidden justify-center text-[12px] tabular-nums text-muted-foreground md:flex",
                            late && "font-semibold text-destructive",
                          )}
                        >
